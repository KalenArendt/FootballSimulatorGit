...
public class FootballScript : MonoBehaviour
{
   ...
   [SerializeField] private GameObject ScoreLeft, ScoreRight;
   [SerializeField] private GameObject Winpanel, LosePanel;

   [SerializeField] private TeamScript homeTeam, awayTeam;
   ...
   {
      var z_pos = Z_AxisManager.transform.position.y / 5f;
   ...
   {
      var ScoringTeamLeft = transform.position.x > 0;
      ...
      for (var i = 0; i < scoreUI.Length; i++)
      {
   ...
   public float CalculateErrorAngle (float Attribute, float targetDistance)
   {
      ...
      var ErrorAngle = Mathf.Asin(Mathf.Pow(UnityEngine.Random.Range(0f, 1f), Attribute / targetDistance)) * Mathf.Rad2Deg;
   ...
   public float TimeToCoverDistance (Vector2 A_pos, Vector2 B_pos, float kickPower)
   {
      ...
      // Initial distance
      var startDistance = Vector2.Distance(B_pos, A_pos);

      // Drag multiplier (friction)
      var rDrag = Mathf.Clamp01(1.0f - (BallRigidbody.drag * Time.fixedDeltaTime));
      ...
      if (velocityPerFrame != Vector2.zero)
      {
         ...
         {
            var dis1 = Vector2.Distance(pPos, B_pos);

            velocityPerFrame *= rDrag;
            pPos += velocityPerFrame;

            var dis2 = Vector2.Distance(pPos, B_pos);
   ...
   public Vector2 FuturePosition (float time)
   {
      ...
      // How many frames are going to pass in the given time
      for (var i = 0; i < time / Time.fixedDeltaTime; i++)
      {
         velocityPerFrame *= rDrag;
