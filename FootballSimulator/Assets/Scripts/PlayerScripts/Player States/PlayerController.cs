...
public class PlayerController : MonoBehaviour
{
    ...
    Transition<PlayerController>.condition cond_kickBall, cond_chaseBall, cond_Wait, cond_dribble, cond_returnHome,
    ...
    State<PlayerController> enumToState(States myState)
    {
    ...
    Transition<PlayerController>.condition enumToCondition(Conditions myCondition)
    {
    ...
    public bool OpponenetsForward()
    {
        ...
        foreach(Collider2D enemy in opponents)
        {
            ...
            float dot = Vector2.Dot(toOpponent, transform.position);
    ...
    bool IsTouchingBall()
    {
        bool touchBall = PlayerCollider.IsTouchingLayers(LayerMask.GetMask("Football")) && GetFootball().transform.localScale.x <= 0.25f;
        return touchBall;
    }
    ...
    public void PassToTarget(PlayerController target)
    {
        if (PlayerCollider.IsTouchingLayers(LayerMask.GetMask("Football")))
        {
            ...
            Vector2 kickDir = new Vector2(target.transform.position.x - GetFootball().transform.position.x, target.transform.position.y - GetFootball().transform.position.y);
                            ...
                            foreach(GameObject currentPlayer in playerTeam.GetTeamPlayers())
                            {
                                    ...
                                    Vector3.Distance(controllingPlayer.transform.position, controllingPlayer.GetComponent<PlayerController>().GoalTarget.transform.position) < 2.0f))
                                {
                                    ...
                                    if (playerTeam.lhs_regions)
                                    {
                                        if (l_dir.x > 0)
                                        {
                                            float dis = Vector2.Distance(currentPlayer.transform.position, controllingPlayer.transform.position);
