...
public class PlayerKickBallState : State<PlayerController>
{
   ...
   private Vector2 KickError (PlayerController player, Vector2 Target, float attribute)
   {
      // Distance to target affects accuracy
      var targetDistance = Vector2.Distance(Target, player.GetFootball().transform.position);

      // Calculate the error angle based on players' ability, distance and a random error factor
      var ErrorAngle = player.GetFootball().GetComponent<FootballScript>().CalculateErrorAngle(attribute, targetDistance);
   ...
   public override void Execute (PlayerController player)
   {
      ...
      var dot = Vector2.Dot(player.transform.up, ToBall.normalized);
      ...
      var power = player.GetComponent<PlayerAttributes>().SHO_Power * dot ;
      ...
      var potShot = Random.Range(0.0f, 10.0f);

      var potShotLimit = 0.1f;
