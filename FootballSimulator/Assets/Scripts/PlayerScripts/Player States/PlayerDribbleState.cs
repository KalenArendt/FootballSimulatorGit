...
public class PlayerDribbleState : State<PlayerController>
{
   ...
   public override void Execute (PlayerController player)
   {
      ...
      var dot = Vector3.Dot(player.transform.up, player.GoalTarget.transform.position);
      ...
      if (dot < 0)
      {
         if (player.GetFootball().GetComponent<Rigidbody2D>().velocity.magnitude < 1f)
         {
            ...
            var kickAngle = 10f * Time.deltaTime * 400;
            ...
            // Kick the ball
            var kickingForce = 0.25f * Time.deltaTime * 400;
      ...
      else
      {
         if (player.GetFootball().GetComponent<Rigidbody2D>().velocity.magnitude < 1f)
         {
            ...
            // Kick the ball
            var kickingForce = 4f * Time.deltaTime * 400;
