...
public class SoccerPitchScript : MonoBehaviour
{
   ...
   private int v_regions = 5;
   private int h_regions = 10;
   ...
   private void InitRegions ()
   {
      ...
      var reg_height = (PitchRegion.topLeft.y - PitchRegion.botRight.y) / v_regions;
      var reg_width = (PitchRegion.botRight.x - PitchRegion.topLeft.x) / h_regions;

      for (var i = 0; i < v_regions; i++)
      {
         for (var j = 0; j < h_regions; j++)
         {
            var sub_region = new Region
            {
               topLeft = new Vector2(PitchRegion.topLeft.x + (j * reg_width), PitchRegion.topLeft.y - (i * reg_height))
            };
            ...
            Instantiate(Test, sub_region.center, new Quaternion(0, 0, 0, 1), transform);
