...
public class TeamScript : MonoBehaviour
{
   ...
   private Transition<TeamScript>.condition cond_HasBall, cond_PrepareForKickOff, cond_AllPlayersHome;
   #endregion
   ...
   private State<TeamScript> enumToState (States myState)
   {
      return myState.ToString() switch
      {
         "PrepareForKickOff" => state_PrepareForKickOff,
         "Defending" => state_Defending,
   ...
   private Transition<TeamScript>.condition enumToCondition (Conditions myCondition)
   {
      return myCondition.ToString() switch
      {
         "HasBall" => cond_HasBall,
         "AllPlayersHome" => cond_AllPlayersHome,
         "PrepareForKickOff" => cond_PrepareForKickOff,
         _ => null,
      };
   }
   #endregion
   ...
   // Team Components
   #region
   private GameObject[] teamPlayers = new GameObject[5];
   ...
   public GameObject FindClosest (GameObject target, bool avoidControllingPPlayer)
   {
      ...
      foreach (GameObject currentPlayer in teamPlayers)
      {
         ...
         {
            var distanceToTarget = Vector2.Distance(currentPlayer.transform.position, target.transform.position);
   ...
   public IEnumerator BSSCoroutine ()
   {
      ...
      foreach (GameObject currentSpot in supportSpots)
      {
         ...
         if (ControllingPlayer != null)
         {
            ...
            var optimalDistance = 3.0f;

            var distance = Vector2.Distance(ControllingPlayer.transform.position, currentSpot.transform.position);

            var temp = Mathf.Abs(optimalDistance - distance);
   ...
   private bool isPassSafeFromOpponent (Vector2 from, Vector2 target, GameObject Receiver, GameObject opponent, float PassingForce)
   {
      ...
      var fromPos3 = new Vector3(from.x, from.y, 0.0f);
      var targetPos3 = new Vector3(target.x, target.y, 0.0f);
      ...
      var TimeForBall = FootBall.GetComponent<FootballScript>().TimeToCoverDistance(from, Projection, PassingForce);

      var maxDistanceTravelled = Vector3.MoveTowards(opponent.transform.position, Projection, Time.deltaTime).normalized.magnitude;

      var reach = TimeForBall + (opponent.GetComponentInChildren<CircleCollider2D>().radius * 0.075f) /*+ FootBall.GetComponent<CircleCollider2D>().radius*/;
   ...
   private bool IsPassSafe (Vector2 from, Vector2 TargetPos, GameObject receiver, float power)
   {
      GameObject[] players = GameObject.FindGameObjectsWithTag("Player");
      var opponents = new List<GameObject>();
   ...
   public bool CanScore (Vector2 from, float power, ref Vector2 ShotTarget)
   {
      ...
      for (var i = numAttempts; i > 0; i--)
      {
         ...
         var TimeForBall = FootBall.GetComponent<FootballScript>().TimeToCoverDistance(from, ShotTarget, power);
                       ...
                       float minPassDistance)
   {
      ...
      foreach (GameObject currentPlayer in teamPlayers)
      {
         ...
         var thisDTP = Vector2.Distance(Passer.transform.position, currentPlayer.transform.position);
         ...
         if (currentPlayer != Passer.gameObject && currentPlayer.gameObject.activeInHierarchy == true && thisDTP >= minPassDistance)
         {
            ...
            var teammateDTG = Vector2.Distance(currentPlayer.GetComponent<PlayerController>().GoalTarget.transform.position, currentPlayer.transform.position);
